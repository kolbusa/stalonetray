...
void reparent_notify(XReparentEvent ev)
{
	...
	if (tmp->l.p == ev.parent) {
		if (!tmp->embedded) {
			DBG(3, "0x%x: the embedding is accomplished\n", tmp->w);
			tmp->embedded = 1;
#ifdef DEBUG
			print_icon_data(tmp);
#endif
...
int main(int argc, char** argv)
{
	...
	tray_set_wm_hints(); /* works around buggy wms */

	XMoveWindow(tray_data.dpy, tray_data.tray, tray_data.xsh.x, tray_data.xsh.y);
	XFlush(tray_data.dpy);
	XSync(tray_data.dpy, False);

/*	ewmh_add_window_state(tray_data.dpy, tray_data.tray, TrayXAtom("_NET_WM_STATE_ABOVE"));*/

#ifdef DEBUG
	ewmh_dump_window_states(tray_data.dpy, tray_data.tray);
#endif
