...
void xembed_switch_focus_to(struct TrayIcon *tgt, long focus)
{
	...
	if (tgt != NULL) {
		...
		DBG(5, ("focus set to icon 0x%x (pointer %p)\n", tgt->wid, tgt));
	} else {
...
/*    unsigned long focus;*/
	...
	if (ev.window != tray_data.tray) {
		DBG(5, ("not handling _XEMBED message which was sent to some other window\n"));
		return;
	}
...
void xembed_del_accel(long id)
{
	...
	if (tgt == NULL) {
		DBG(1, ("refusing to remove unregistered accelerator\n"));
		return;
	}
