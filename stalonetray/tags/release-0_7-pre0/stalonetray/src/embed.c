...
int embedder_embed(struct TrayIcon *ti)
{
	...
	DBG(6, ("position of the icon inside the tray: (%d, %d)\n", x, y));
...
static int embedder_update_window_position(struct TrayIcon *ti)
{
	...
	if (!x11_ok()) {
		DBG(0, ("failed to update the position of 0x%x\n", ti->wid));
		ti->is_invalid = True;
	}
...
int embedder_refresh(struct TrayIcon *ti)
{
	...
	/* Send Expose event asking (again) icon to repaint itself */
	xe.type = Expose;
	...
	/* Clear icon window before sending the event */
	...
	if (!x11_ok()) {
		DBG(0, ("could not ask  0x%x for repaint\n", ti->wid));
		ti->is_invalid = True;
	}
