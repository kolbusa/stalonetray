...
/************ CLI **************/
/* Types of command line parameter targets */
#define CLP_STRING	1
#define CLP_INT		2
#define CLP_FLAG	3
...
struct Param params[] = {
	...
	{NULL, "--fuzzy-edges", "fuzzy_edges", &settings.fuzzy_edges, (param_parser_t) &parse_int, 1, 1, 1, "2"},
...
void usage(char *progname) 
{
	...
	printf( "\n"
			...
			"    -d, --decorations [<deco>] set what window decorations are visible;\n"
			"                               deco can be: none, title, border, all\n"
			...
			"    --ignore-icon-resize       ignore icons attempts resize their windows\n"
			...
			if (p->takes_arg) {
				if (p->short_name != NULL && strstr(*argv, p->short_name) == *argv) {
					...
					if ((*argv)[strlen(p->long_name)] == '=') /* accept arguments in the form --abcd=5 */
						...
						if (argc > 1 && argv[1][0] != '-' ) { /* there's an argument (same XXX as above) */
							arg = *(++argv); 
							argc--;
						} else if (!p->optional_arg) { /*argument is missing */
							ERR(("%s expects an argument\n", p->long_name));
...
void interpret_settings()
{
	...
	if (!x11_ok() || !rc) {
		ERR(("bad bacground color specification \"%s\"\n", settings.bg_color_str));
		settings.bg_color.pixel = 0x777777; /* ugly gray color */
	}
	...
	if (settings.tint_level) {
		...
		if (!x11_ok() || !rc) {
			ERR(("bad tint color specification \"%s\"\n", settings.bg_color_str));
			settings.tint_level = 0; /* disable tinting */
		}
	}
	...
	DBG(4, ("final geometry: %dx%d at (%d,%d)\n", 
			tray_data.xsh.width, tray_data.xsh.height,
			tray_data.xsh.x, tray_data.xsh.y));
