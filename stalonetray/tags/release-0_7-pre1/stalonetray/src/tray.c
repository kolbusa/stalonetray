...
#define recreate_pixmap(pmap,gc) do { \
			...
			(settings.transparent || settings.fuzzy_edges)) 
	{
		if (!tray_update_root_bg_pmap(&root_pmap, NULL, NULL)) {
			...
			DBG(10, ("still waiting for root pixmap\n"));
#endif
			return SUCCESS;
		} else
...
void tray_create_window(int argc, char **argv)
{
	...
	*xsh = tray_data.xsh;
	...
	/* XXX Do we need all these masks */
	XSelectInput(tray_data.dpy, tray_data.tray, StructureNotifyMask | FocusChangeMask | PropertyChangeMask | ExposureMask | VisibilityChangeMask);
