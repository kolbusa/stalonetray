...
int layout_handle_icon_resize(struct TrayIcon *ti)
{
	...
	if (ti->l.layed_out) {
		...
		window2grid(ti);
		if (ti->l.grd_rect.w == l.grd_rect.w &&
		    ti->l.grd_rect.h == l.grd_rect.h)
		{
			return SUCCESS;
		}
	}
	...
	if (ti->l.layed_out) {
		...
		if (l.grd_rect.w != ti->l.grd_rect.w || 
		    l.grd_rect.h != ti->l.grd_rect.h) 
		{
			rebuild_grid(ti);
		}
...
int create_icon_placement(struct IconPlacement *ip, int x, int y, struct Layout *l)
{
	ip->valid = 0;
	...
	ip->ggrow.x = x + l->grd_rect.w - grid_sz.x;
	ip->ggrow.y = y + l->grd_rect.h - grid_sz.y;
...
struct IconPlacement *find_placement(struct Layout *l)
{
	...
	if (grid_sz.x == 0 || grid_sz.x == 0) { 	/* grid is empty */
		create_icon_placement(&cur_pmt, 0, 0, l);
	} else {									/* seek and place */
		create_icon_placement(&cur_pmt, grid_sz.x, 0, l);
		create_icon_placement(&tmp_pmt, 0, grid_sz.y, l);
		choose_best_placement(&cur_pmt, &tmp_pmt);
		do {
			skip = grid_check_rect_free(x, y, l->grd_rect.w, l->grd_rect.h);
			DBG(8, ("x=%d, y=%d, skip=%d\n", x, y, skip));
			if (skip == 0 && create_icon_placement(&tmp_pmt, x, y, l)) {
				choose_best_placement(&cur_pmt, &tmp_pmt);
				if (!cur_pmt.ggrow.x && !cur_pmt.ggrow.y) break;
				skip = l->grd_rect.w;
			}
			x += skip;
			if (x >= grid_sz.x) {
...
int trayicon_cmp_func(struct TrayIcon *ti1, struct TrayIcon *ti2)
{
	return (!ti2->l.layed_out && ti1->l.layed_out) || 
...
{
	int layed_out;
	/* can anyone guarantee left-to-right evalueation ? */
	layed_out = ti->l.layed_out;
